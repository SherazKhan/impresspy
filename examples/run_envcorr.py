...
labels =read_labels_from_annot(subject, 'aparc', 'both', subjects_dir=subjects_dir)
...
raw.filter(None, 45, l_trans_bandwidth='auto', h_trans_bandwidth='auto',
...
labels_fname = [str(labels_path +  label.name[-2:] + '.' +label.name[:-3] + '.label')  for label in labels]
...
brain.save_image('beta_orthogonal_corr_n.png')
